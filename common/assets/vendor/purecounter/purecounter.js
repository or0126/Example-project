const CLUB_INFO_URL = buildUrlWithContextPath("info");

async function fetchClubInfo() {
    const response = await fetch(CLUB_INFO_URL);
    const clubInfoDTO = await response.json();

    document.getElementById('membersCount').dataset.purecounterEnd = clubInfoDTO.numOfMembers;
    document.getElementById('boatsCount').dataset.purecounterEnd = clubInfoDTO.numOfBoats;
    document.getElementById('assignmentsCount').dataset.purecounterEnd = clubInfoDTO.numOfFutureAssignments;
    document.getElementById('activitiesCount').dataset.purecounterEnd = clubInfoDTO.numOfActivities;
}
            ...
            value: async function () {
                ...
                if (this.intersectionListenerSupported())
                    for (var n = new IntersectionObserver(this.animateElements.bind(this), {
                        ...
                        rootMargin: "20px",
                        threshold: .5
                    }), r = 0; r < t.length; r++) n.observe(t[r]);
